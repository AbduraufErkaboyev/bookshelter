...
let logout = document.querySelector(".logout"),
    searchInput = document.querySelector(".search_inputt"),
    result = document.querySelector(".result_count"),
    controlSort = document.querySelector(".control_sort"),
    bookmarks = document.querySelector(".bookmarks_part"),
...
function render(booksArray, list) {
    ...
    booksArray.forEach((book) => {
        ...
        let newBookName = document.createElement("h1");
...
function all(searchValue) {
    function mainFetch(searchValue) {
        fetch(`https://www.googleapis.com/books/v1/volumes?q=${searchValue}&startIndex=1`).then((res) => res.json()).then((data) => {
    ...
    function mainFetchP(searchValue) {
        ...
        pagenation.addEventListener("click", (e) => {
            ...
            if (target.className === "pr_btn fa-sharp fa-solid fa-arrow-left-long") {
                if (currentPage > 1) {
                    fetch(`https://www.googleapis.com/books/v1/volumes?q=${searchValue}&startIndex=${currentPage - 1}`)
                        .then((res) => res.json())
                        .then((data) => {
                            render(data.items, books);
                            window.localStorage.setItem("array", JSON.stringify(data.items));
                ...
                fetch(`https://www.googleapis.com/books/v1/volumes?q=${searchValue}&startIndex=${target.id}`)
                    .then((res) => res.json())
                    .then((data) => {
                        render(data.items, books);
                        window.localStorage.setItem("array", JSON.stringify(data.items));
...
function all2(searchValue) {
    function mainFetch(searchValue) {
        fetch(`https://www.googleapis.com/books/v1/volumes?q=${searchValue}&orderBy=newest&startIndex=1`).then((res) => res.json()).then((data) => {
            render(data.items, books);
            window.localStorage.setItem("array", JSON.stringify(data.items));
    ...
    function mainFetchP(searchValue) {
        ...
        pagenation.addEventListener("click", (e) => {
            ...
            if (target.className === "pr_btn fa-sharp fa-solid fa-arrow-left-long") {
                if (currentPage > 1) {
                    fetch(`https://www.googleapis.com/books/v1/volumes?q=${searchValue}&orderBy=newest&startIndex=${currentPage - 1}`)
                        .then((res) => res.json())
                        .then((data) => {
                            render(data.items, books);
                            window.localStorage.setItem("array", JSON.stringify(data.items));
                ...
                fetch(`https://www.googleapis.com/books/v1/volumes?q=${searchValue}&orderBy=newest&startIndex=${target.id}`)
                    .then((res) => res.json())
                    .then((data) => {
                        render(data.items, books);
                        window.localStorage.setItem("array", JSON.stringify(data.items));
...
let bookmarksArrayLocal = JSON.parse(window.localStorage.getItem("bookmarksBook"))
...
books.addEventListener("click", (e) => {
    ...
    if (target.className === "more") {
        ...
        window.localStorage.setItem("bookmarksBook", JSON.stringify(bookmarksArray));
        renderBookmark();
    }
});
...
function renderBookmark() {
    ...
    <div class="icons">
        <a href = "${book.volumeInfo.previewLink}"><img src="img/book-open 1.png" alt=""></a>
        <img id = "${book.id}" class = "deleteBookmark" src="img/delete 1.png" alt="" n>
    </div>
    </div>`
...
function activePage(p) {
    ...
    let [restPage] = pagesArray.filter((page) => page.classList.contains("active"));
